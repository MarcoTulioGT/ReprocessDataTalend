...
pipeline {
    ...
    stages {
        ...
        stage('Parametrizando'){
           ...
           //  try {
            input = new FileInputStream("/var/lib/jenkins/workspace/Test_MQ@script/Test_MQ.properties");
            prop.load(input);
	...
	// } finally {
		// if (input != null) {
			...
			// 	e.printStackTrace();
             ...
             def json = readFile(file: '/var/lib/jenkins/workspace/Test_MQ@script/Test_MQ.json')
